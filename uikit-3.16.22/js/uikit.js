...
(function (global, factory) {
  ...
  const camelize = memoize((str) =>
    (str.charAt(0).toLowerCase() + str.slice(1)).replace(camelizeRe, (_, c) =>
      c.toUpperCase()
    )
  );
  ...
  function startsWith(str, search) {
    var _a;
    return (_a = str == null ? void 0 : str.startsWith) == null
      ? void 0
      : _a.call(str, search);
  }
  function endsWith(str, search) {
    var _a;
    return (_a = str == null ? void 0 : str.endsWith) == null
      ? void 0
      : _a.call(str, search);
  }
  function includes(obj, search) {
    var _a;
    return (_a = obj == null ? void 0 : obj.includes) == null
      ? void 0
      : _a.call(obj, search);
  }
  function findIndex(array, predicate) {
    var _a;
    return (_a = array == null ? void 0 : array.findIndex) == null
      ? void 0
      : _a.call(array, predicate);
  }
  ...
  function sumBy(array, iteratee) {
    return array.reduce(
      (sum, item) =>
        sum + toFloat(isFunction(iteratee) ? iteratee(item) : item[iteratee]),
      0
    );
  }
  function uniqueBy(array, prop) {
    const seen = /* @__PURE__ */ new Set();
    return array.filter(({ [prop]: check }) =>
  ...
  function isVoidElement(element) {
    return toNodes(element).some(
      (element2) => voidElements[element2.tagName.toLowerCase()]
    );
  }
  function isVisible(element) {
    return toNodes(element).some(
      (element2) =>
        ...
        element2.getClientRects().length
    );
  }
  ...
  function closest(element, selector) {
    return isElement(element)
      ? element.closest(
          startsWith(selector, ">") ? selector.slice(1) : selector
        )
      : toNodes(element)
          .map((element2) => closest(element2, selector))
          .filter(Boolean);
  }
  ...
  const splitSelector = memoize((selector) =>
    selector
      .match(selectorRe)
      .map((selector2) => selector2.replace(/,$/, "").trim())
  );
  ...
  function trigger(targets, event, detail2) {
    return toEventTargets(targets).every((target) =>
      target.dispatchEvent(createEvent(event, true, true, detail2))
    );
  }
  ...
  function getEventPos(e) {
    ...
    const { clientX: x, clientY: y } =
      ((_a = e.touches) == null ? void 0 : _a[0]) ||
      ((_b = e.changedTouches) == null ? void 0 : _b[0]) ||
      e;
    return { x, y };
  }
  ...
  function css(element, property, value, priority) {
    ...
    for (const element2 of elements) {
      if (isString(property)) {
        ...
        priority = value;
        each(property, (value2, property2) =>
          css(element2, property2, value2, priority)
        );
      }
    }
    return elements[0];
        ...
        (element2) =>
          new Promise((resolve, reject) => {
            ...
            const timer = setTimeout(
              () => trigger(element2, "transitionend"),
              duration
            );
            once(
              element2,
  ...
  function isTag(element, ...tagNames) {
    return tagNames.some((tagName) => {
      var _a;
      return (
        ((_a = element == null ? void 0 : element.tagName) == null
          ? void 0
          : _a.toLowerCase()) === tagName.toLowerCase()
      );
    });
  }
  ...
  function html(parent2, html2) {
    return isUndefined(html2)
      ? $(parent2).innerHTML
      : append(empty(parent2), html2);
  }
  ...
  function wrapInner(element, structure) {
    return toNodes(
      toNodes(element).map((element2) =>
        element2.hasChildNodes()
          ? wrapAll(toArray(element2.childNodes), structure)
          : append(element2, structure)
      )
    );
  }
  function unwrap(element) {
    toNodes(element)
      ...
      .forEach((parent2) => parent2.replaceWith(...parent2.childNodes));
  }
  ...
  function offsetPosition(element) {
    ...
    while ((element = element.offsetParent)) {
      ...
      offset2[1] +=
        element.offsetLeft + toFloat(css(element, `borderLeftWidth`));
  ...
  function boxModelAdjust(element, prop, sizing = "border-box") {
    return css(element, "boxSizing") === sizing
      ? sumBy(
          ...
          (prop2) =>
            toFloat(css(element, `padding${prop2}`)) +
            toFloat(css(element, `border${prop2}Width`))
        )
      : 0;
  }
  ...
  const pointerDown$1 = hasPointerEvents
    ? "pointerdown"
    : hasTouch
    ? "touchstart"
    : "mousedown";
  const pointerMove$1 = hasPointerEvents
    ? "pointermove"
    : hasTouch
    ? "touchmove"
    : "mousemove";
  const pointerUp$1 = hasPointerEvents
    ? "pointerup"
    : hasTouch
    ? "touchend"
    : "mouseup";
    ...
    return (el[stateKey] = new Promise((resolve) => {
      ...
      once(window, "message", resolve, false, ({ data }) => {
        try {
          ...
          return (
            (youtube &&
              (data == null ? void 0 : data.id) === id &&
              data.event === "onReady") ||
            (vimeo && Number(data == null ? void 0 : data.player_id) === id)
          );
        } catch (e) {}
      });
  ...
  function scrollIntoView(element, { offset: offsetBy = 0 } = {}) {
    const parents2 = isVisible(element)
      ? scrollParents(element, false, ["hidden"])
      : [];
    return parents2.reduce(
      (fn, scrollElement, i) => {
        ...
        const { height: elHeight, top: elTop } = parents2[i - 1]
          ? offsetViewport(parents2[i - 1])
          : offset(element);
  ...
  function attachTo(element, target, options) {
    ...
    for (const [i, [prop, , start, end]] of Object.entries(dirs)) {
      const targetOffset =
        attach.target[i] === attach.element[i]
          ? offsetViewport(target[i])
          : offset(target[i]);
  ...
  function getViewport$2(element, target, viewportOffset, boundary, i) {
    let viewport = getIntersectionArea(
      ...commonScrollParents(element, target).map(offsetViewport)
    );
    if (viewportOffset) {
  ...
  function registerEvent(instance, event, key) {
    let { name, el, handler, capture, passive, delegate, filter, self } =
      isPlainObject(event) ? event : { name: key, handler: event };
  ...
  function registerObservable(instance, observable) {
    ...
    if (isFunction(target) && isArray(instance[key]) && observer.unobserve) {
      registerWatch(
        instance,
        { handler: updateTargets(observer), immediate: false },
        key
      );
    }
  ...
  function toList(value) {
    return isArray(value)
      ...
      ? value
          .split(/,(?![^(]*\))/)
          .map((value2) =>
            isNumeric(value2) ? toNumber(value2) : toBoolean(value2.trim())
          )
      : [value];
  }
    ...
    const observer = new MutationObserver((records) => {
      ...
      if (
        records.some(({ attributeName }) => {
          ...
          return (
            prop === id ? attributes : [camelize(prop), camelize(attributeName)]
          ).some(
            (prop2) =>
              !isUndefined(data[prop2]) && data[prop2] !== $props[prop2]
          );
        })
      ) {
        instance.$reset();
  ...
  function callHook(instance, hook) {
    ...
    (_a = instance.$options[hook]) == null
      ? void 0
      : _a.forEach((handler) => handler.call(instance));
  }
  ...
  function component(name, options) {
    ...
    if (App._initialized && !opt.functional) {
      requestAnimationFrame(() =>
        createComponent(name, `[${id}],[data-${id}]`)
      );
    }
  ...
  function globalApi(App) {
    ...
    App.mixin = function (mixin, component2) {
      component2 =
        (isString(component2) ? this.component(component2) : component2) ||
        this;
    ...
    computed: {
      ...
      hasTransition({ animation }) {
        return ["slide", "reveal"].some((transition) =>
          startsWith(animation[0], transition)
        );
      },
    },
    methods: {
        ...
        if (this.cls) {
          ...
          changed &&
            toggleClass(
              el,
              this.cls,
              includes(this.cls, " ") ? void 0 : toggled
            );
        } else {
          changed = toggled === el.hidden;
          changed && (el.hidden = !toggled);
        }
        $$("[autofocus]", el).some((el2) =>
          isVisible(el2) ? el2.focus() || true : el2.blur()
        );
        if (changed) {
  ...
  async function toggleTransition(
    ...
    const [mode = "reveal", startProp = "top"] =
      ((_a = animation[0]) == null ? void 0 : _a.split("-")) || [];
    const dirs = [
      ["left", "right"],
      ["top", "bottom"],
    ];
    ...
    return intersection({
      handler: function (entries, observer) {
        ...
        for (const el of toNodes(
          isFunction(targets) ? targets(this) : targets
        )) {
          $$('[loading="lazy"]', el)
            .slice(0, preload - 1)
            .forEach((el2) => removeAttr(el2, "loading"));
        }
    ...
    computed: {
      ...
      contents({ content }) {
        return this.items.map((item) => {
          var _a;
          return (
            ((_a = item._wrapper) == null ? void 0 : _a.firstElementChild) ||
            $(content, item)
          );
        });
      },
    ...
    update() {
      ...
      for (const index2 in this.items) {
        ...
        attr(toggle, {
          ...
          "aria-disabled":
            !this.collapsible && activeItems.length < 2 && active,
        });
    ...
    methods: {
      toggle(item, animate) {
        ...
        if (
          ...
          includes(activeItems, item)
        ) {
          return;
        }
        return Promise.all(
          items.map((el) =>
            this.toggleElement(el, !includes(activeItems, el), (el2, show) => {
  ...
  async function transition(
    el,
    show,
    { content, duration, velocity, transition: transition2 }
  ) {
    var _a;
    content =
      ((_a = el._wrapper) == null ? void 0 : _a.firstElementChild) ||
      $(content, el);
    if (!el._wrapper) {
    ...
    duration =
      (velocity * endHeight + duration) * (show ? 1 - percent : percent);
    ...
    methods: {
      ...
      getPositionOffset(element) {
        return (
          toPx(
            this.offset === false
              ? css(element, "--uk-position-offset")
              : this.offset,
    ...
    const off = on(
      ...
      (e) => {
        if (
          ...
          matches(e.target, 'input[type="range"')
        ) {
          return;
        }
              ...
              [prop]:
                (targetOffset[start] > elOffset[start]
                  ...
                  : Math.min(
                    ...
                    ) - targetOffset[this.inset ? start : end]) -
                positionOffset,
              [`overflow-${axis}`]: "auto",
            });
            this.positionAt(this.$el, this.target, this.boundary);
  ...
  function getViewport$1(el, target) {
    return offsetViewport(
      overflowParents(target).find((parent2) => within(el, parent2))
    );
  }
  ...
  function listenForScroll(drop) {
    return on(
      [document, ...overflowParents(drop.$el)],
      "scroll",
      () => drop.$emit(),
      {
        passive: true,
  ...
  var Dropnav = {
      ...
      dropdowns({ clsDrop }, $el) {
        ...
        if (this.dropContainer !== $el) {
          for (const el of $$(`.${clsDrop}`, this.dropContainer)) {
            const target =
              (_a = this.getDropdown(el)) == null ? void 0 : _a.targetEl;
            if (
    ...
    events: [
      {
        ...
        handler({ current }) {
          ...
          if (
            active2 &&
            includes(active2.mode, "hover") &&
            active2.targetEl &&
            !within(active2.targetEl, current) &&
            !active2.isDelaying
          ) {
            active2.hide(false);
          }
      ...
      },
      {
        ...
        handler(e) {
          ...
          if (
            keyCode === keyMap.DOWN &&
            (active2 == null ? void 0 : active2.targetEl) === current
          ) {
            e.preventDefault();
        ...
        handler({ target }) {
          ...
          const adjustHeight = () => {
            ...
            const maxBottom = Math.max(
              ...targetOffsets.map(({ bottom }) => bottom)
            );
            const dropbarOffset = offset(this.dropbar);
            css(
              this.dropbar,
              "top",
              this.dropbar.offsetTop - (dropbarOffset.top - minTop)
            );
            this.transitionTo(
      ...
      },
      {
        ...
        handler(e) {
          ...
          if (
            ...
            active2.$el === e.target &&
            !this.items.some(
              (el) => active2.targetEl !== el && matches(el, ":focus")
            )
          ) {
            e.preventDefault();
          }
    ...
    methods: {
      getActive() {
        var _a;
        return (
          includes(this.dropdowns, (_a = active$1) == null ? void 0 : _a.$el) &&
          active$1
        );
      },
      async transitionTo(newHeight, el) {
        ...
        css(
          el,
          "clipPath",
          `polygon(0 0,100% 0,100% ${oldHeight}px,0 ${oldHeight}px)`
        );
  ...
  function handleNavItemNavigation(e, toggles, active2) {
    ...
    if (~next) {
      ...
      toggles[
        getIndex(next, toggles, toggles.indexOf(active2.targetEl || current))
      ].focus();
    }
  }
  ...
  var formCustom = {
    ...
    update() {
      ...
      const value = ((_a = input.files) == null ? void 0 : _a[0])
        ? input.files[0].name
    ...
    update: {
      read() {
        ...
        if (this.expand) {
          minHeight = `${
            viewportHeight -
            (dimensions$1(scrollElement).height -
          ...
          minHeight = `calc(${
            isScrollingElement ? "100vh" : `${viewportHeight}px`
          }`;
          if (this.offsetTop) {
            if (isScrollingElement) {
              const top =
                offsetPosition(this.$el)[0] - offsetPosition(scrollElement)[0];
              minHeight +=
                top > 0 && top < viewportHeight / 2 ? ` - ${top}px` : "";
            } else {
          ...
          if (this.offsetBottom === true) {
            minHeight += ` - ${
              dimensions$1(this.$el.nextElementSibling).height
            }px`;
      ...
      write({ minHeight }) {
        css(
          this.$el,
          "minHeight",
          `max(${this.minHeight || 0}px, ${minHeight})`
        );
      },
      events: ["resize"],
    },
  ...
  var I18n = {
    ...
    methods: {
      t(key, ...params) {
        ...
        return (
          ((_c =
            ((_a = this.i18n) == null ? void 0 : _a[key]) ||
            ((_b = this.$options.i18n) == null ? void 0 : _b[key])) == null
            ? void 0
            : _c.replace(/%s/g, () => params[i++] || "")) || ""
        );
      },
    },
  };
  ...
  const Search = {
    ...
    beforeConnect() {
      ...
      if (
        hasClass(this.$el, "uk-search-toggle") ||
        hasClass(this.$el, "uk-navbar-toggle")
      ) {
  ...
  const ButtonComponent = {
    ...
    beforeConnect() {
      ...
      attr(
        button,
        "role",
        this.role !== null && isTag(button, "a") ? "button" : this.role
      );
  ...
  function setSrcAttrs(el, src) {
    if (isImg(el)) {
      ...
      const elements = isTag(parentNode, "picture")
        ? children(parentNode)
        : [el];
  ...
  var scrollspyNav = {
    ...
    computed: {
      links(_, $el) {
        return $$('a[href*="#"]', $el).filter(
          (el) => el.hash && isSameSiteAnchor(el)
        );
      },
    ...
    update: [
      {
        read({ height: height$1, width, margin, sticky }) {
          ...
          let offset$1 = toPx(
            this.offset,
            "height",
            sticky ? this.$el : referenceElement
          );
          if (
            position === "bottom" &&
            (height$1 < dynamicViewport || this.overflowFlip)
          ) {
            offset$1 += dynamicViewport - height$1;
          }
          const overflow = this.overflowFlip
            ? 0
            : Math.max(0, height$1 + offset$1 - viewport2);
      ...
      },
      {
        ...
        write(data, types) {
          ...
          if (
            scroll2 < 0 ||
            (scroll2 === prevScroll && isScrollUpdate) ||
          ...
          if (
            this.showOnUp &&
            !this.isFixed &&
            Math.abs(data.initScroll - scroll2) <= 30 &&
            Math.abs(prevScroll - scroll2) <= 10
          ) {
            return;
          }
          if (
  ...
  var Switcher = {
    ...
    events: [
      ...
      },
      {
        ...
        handler(e) {
          ...
          if (~i) {
            e.preventDefault();
            const toggles = this.toggles.filter(
              (el) => !matches(el, selDisabled)
            );
            const next =
              toggles[getIndex(i, toggles, toggles.indexOf(current))];
            next.focus();
            if (this.followFocus) {
              this.show(next);
            }
    ...
    update() {
      ...
      for (const index in this.toggles) {
        ...
        const item =
          (_a = this.connects[0]) == null ? void 0 : _a.children[index];
      ...
      attr(
        this.$el,
        "aria-orientation",
        matches(this.$el, this.selVertical) ? "vertical" : null
      );
    },
    methods: {
      index() {
      ...
      async setState(state, animate = true) {
        ...
        for (const toggle of this.toggles) {
          toggleClass(
            toggle,
            this.cls,
            matchFilter(toggle, this.attrItem, state)
          );
        }
        await Promise.all(
  ...
  function sortItems(nodes, sort, order) {
    return [...nodes].sort(
      (a, b) =>
        data(a, sort).localeCompare(data(b, sort), void 0, { numeric: true }) *
        (order === "asc" || -1)
    );
  }
  ...
  function hasSelectableText(el) {
    return (
      ...
      toArray(el.childNodes).some(
        (el2) => el2.nodeType === 3 && el2.textContent.trim()
      )
    );
  }
    ...
    methods: {
      show(index, force = false) {
        ...
        if (!force && stack.length > 1) {
          if (stack.length === 2) {
            (_a = this._transitioner) == null
              ? void 0
              : _a.forward(Math.min(this.duration, 200));
          }
          return;
        }
        ...
        const promise = this._show(prev, next, force).then(() => {
          ...
          this._transitioner = null;
          requestAnimationFrame(
            () => stack.length && this.show(stack.shift(), true)
          );
        });
  ...
  var Parallax = {
    ...
    methods: {
      ...
      getCss(percent) {
        ...
        css2.willChange = Object.keys(css2)
          .filter((key) => css2[key] !== "")
          .map(propName)
          .join(",");
        return css2;
  ...
  function backgroundCoverFn(prop, el, stops, props2) {
    ...
    for (const prop2 of bgProps) {
      ...
      positions[prop2] = `max(${getBackgroundPos(
        el,
        prop2
      )},-${overflow}px) + ${positions[prop2]}`;
    }
  ...
  function setBackgroundPosFn(bgProps, positions, props2) {
    return function (css2, percent) {
      for (const prop of bgProps) {
        ...
        css2[
          `background-position-${prop.substr(-1)}`
        ] = `calc(${positions[prop]} + ${value}px)`;
      }
    };
  }
  const dimensions = {};
  ...
  function getStop(stops, percent) {
    const index =
      findIndex(
        stops.slice(1),
        ([, targetPercent]) => percent <= targetPercent
      ) + 1;
    return [
      stops[index - 1][0],
      stops[index][0],
  ...
  var parallax = {
    ...
    computed: {
      ...
      start({ start }) {
        ...
        return toPx(
            ...
            ((viewport2 = (1 - viewport2) * 100) &&
              `${viewport2}vh+${viewport2}%`),
          "height",
          this.target,
          true
        );
    ...
    observe: [
      resize({
        target: ({ $el, target }) => [$el, target, scrollParent(target, true)],
      }),
      scroll$1(),
      viewport(),
    ],
    ...
    return {
      ...
      show(duration, percent = 0, linear) {
        ...
        return new Promise((res) => {
          ...
          Transition.start(
            list,
            { transform: translate(-to * (isRtl ? -1 : 1.01), "px") },
            duration,
            timing
          ).then(resolve, noop);
        });
        ...
        for (let i = 0; i < this.length; i++) {
          ...
          if (this.center) {
            if (
              ...
              left +
                slideWidth +
                dimensions$1(getIndex(+i + 1, this.slides)).width / 2 >
                width / 2
            ) {
              sets.push(+i);
  ...
  function isFinite(list, center) {
    ...
    if (!center) {
      return (
        Math.ceil(getWidth(list)) < Math.trunc(listWidth + getMaxElWidth(list))
      );
    }
  ...
  var Animations = {
    ...
    scale: {
      show() {
        return [
          { opacity: 0, transform: scale3d(1 + -1), zIndex: 0 },
          { zIndex: -1 },
        ];
      },
      percent(current) {
          ...
          const handlers = [
            once(
              ...
              (e2) =>
                ...
                (e2.type === "keydown" && e2.keyCode === keyMap.ESC)
            ),
